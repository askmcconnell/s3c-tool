...
Output: s3c_inventory_HOSTNAME_DATE.csv  (upload to askmcconnell.com/s3c)

Usage:
    python3 s3c_scan_mac.py
    python3 s3c_scan_mac.py --output /path/to/output.csv
...
# ── Output CSV columns (matches S3C-Tool reference DB schema) ─────────────────────
FIELDNAMES = [
    's3c_format_version',
    'scan_date',
...
def main():
    ...
    output_path = args.output or os.path.expanduser(
        f'~/Desktop/s3c_inventory_mac_{date.today().isoformat()}.csv'
    )
